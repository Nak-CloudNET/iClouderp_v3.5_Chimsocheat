...
/* -----------------------
	 * Product Group Price change
	 ...
	 $(document).on('change', '#pgroup_price', function () {
		...
		if(item.dropdown_group_prices !== false) {
			$.each(item.dropdown_group_prices, function () {
				if(opt ==0) {
					$('#pprice').val(default_price);
					$('#pprice_show').val(default_price);
					item.row.price_id = opt;
					localStorage.setItem('quitems', JSON.stringify(item));
...
$(document).on('click', '.qudel', function () {
			...
			$.each(item.dropdown_group_prices, function () {
				if(gp == 1) {
					...
					if(item.row.price_id == 0) {
						$("<option/>", {value: this.price_group_id, text: this.group_name}).attr("data-currency_code", this.currency_code).appendTo(opt_group_price);
					}else{
						$("<option/>", {value: this.price_group_id, text: this.group_name}).attr("data-currency_code", this.currency_code).appendTo(opt_group_price);
					}
				}else{
	...
	$(document).on('change', '#poption', function () {
	 	...
	 	if(item.options !== false) {
	 		$.each(item.options, function () {
	 			if(this.id == opt && this.price != 0) {
					if(!item.all_group_price || item.row.price_id == 0) {
						...
						$.each(item.all_group_price, function() {
							if(item.options) {
								if(this.product_id == item.row.id && this.price_group_id == item.row.price_id && this.unit_id == opt && this.unit_type == 'variant') {
									$('#pprice,#pprice_show').val(this.price);
									$("#net_price").text(formatMoney(this.price));
								}
							}else {
				...
				if(group_prices){
					...
					$.each(group_prices, function(){
						...
						if(item.makeup_cost == 1){
							...
							if(item.options) {
								if(this.price_group_id == price_id && this.product_id == product_id && this.unit_id == item_option && this.unit_type == 'variant')
								{
