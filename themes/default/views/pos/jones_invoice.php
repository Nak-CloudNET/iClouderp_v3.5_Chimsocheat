    ...
    <div id="receipt-data">
        ...
        <div class="text-center">
			...
			<tr>
					...
					<td style="text-align:right;">$ <?= $this->erp->formatMoney($totals-$inv->product_discount); ?></td>
			</tr>
                <?php 
				$pos_paid = 0;
				$pos_paidd = 0;
				...
				?>
                    ...
                    <tr>
                        ...
                        <th style="border-bottom:2px solid #000;border-right:2px solid #000;border-left:none;" class="text-right"><?= $this->erp->formatMoney($inv->recieve_usd * $inv->other_cur_paid_rate) ; ?><sup> ៛ </sup></th>
                    </tr>
					...
					if(count($payments) > 1){
						...
						if((($pay + ($pay_kh / (($pos_settings->in_out_rate) ? $outexchange_rate->rate : $exchange_rate->rate))) - $totals) != 0){
				...
				?>
					...
					<tr>
						...
						<?php
							echo $this->erp->formatMoney(($pay+$pay_kh) - $totals);
							$total_us_b = $this->erp->formatMoney(($pay+$pay_kh) - $totals);
                    ...
                    <tr>
						...
						<th style="border-top:2px dotted #000" class="text-right"><?= number_format(round((($pay+$pay_kh) - $totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate)), -3) . '៛' ; ?></th>
                    </tr>
				<?php
						}
					}else{
						// $this->erp->print_arrays($inv);
					if((($pos_paid+$amount_kh_to_us) - ($inv->recieve_usd - $totals)) != 0 || ($this->erp->formatMoney((($pos_paid+$amount_kh_to_us) - $totals) * $exchange_rate->rate)) != 0) { ?>
					<tr>
				...
				?>
					<tr>
						...
						<?php
							...
							echo '$ '. $this->erp->formatMoney(abs(($pay+$money_kh) - $totals));
							$total_us_b = $this->erp->formatMoney(($pay+$money_kh) - $totals);
							$m_us = $this->erp->fraction($total_us_b);
							//echo $m_us;
						?>
						</th>
                    ...
                    <tr>
                        ...
                        <th style="border-top:2px dotted #000" class="text-right"><?= number_format(abs((($pay+$money_kh) - $totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate))) . ' ៛' ; ?></th>
                    </tr>
				<?php
						}
					}else{
					if((($pos_paid+$amount_kh_to_us) - $totals) != 0 || ($this->erp->formatMoney((($pos_paid+$amount_kh_to_us) - $totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate) )) != 0){ ?>
                    <tr>
                        <th style="border-top:2px dotted #000;padding-right: 92px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("remaining_us"); ?> :</th>
                        <th style="border-top:2px dotted #000" class="text-right">
						<?php
							echo '$ '. $this->erp->formatMoney(abs(($pos_paid+$amount_kh_to_us) - $totals));
							$total_us_b = $this->erp->formatMoney(($pos_paid+$amount_kh_to_us) - $totals);
							$m_us = $this->erp->fraction($total_us_b);
						?>
						</th>
                    </tr>
                    <tr>
                        <th style="border-top:2px dotted #000;padding-right: 92px;" colspan="<?= $colspan ?>" class="text-right"><?= lang("remaining_kh"); ?> :</th>
                        <th style="border-top:2px dotted #000" class="text-right"><?= number_format(abs((($pos_paid+$amount_kh_to_us) - $totals)*(($pos_settings->in_out_rate) ? $outexchange_rate->rate:$exchange_rate->rate))) . ' ៛' ; ?></th>
                    </tr>
				<?php }
					}
				}
