    ...
    function loadItems() {
        ...
        if (localStorage.getItem('reitems')) {
            ...
            $.each(reitems, function () {
                ...
                var item_type = item.row.type, product_id = item.row.id, combo_items = item.combo_items, sale_item_id = item.row.sale_item_id, item_option = item.row.option, item_price = item.row.price, qty_returned = item.row.qty_returned, item_qty = item.row.qty, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
                ...
                if (ds.indexOf("%") !== -1) {
                    ...
                    if (!isNaN(pds[0])) {
                        ...
                        item_discount = parseFloat(parseFloat(((unit_price) * parseFloat(pds[0])) / 100));
                    } else {
                       
                        item_discount = parseFloat(ds/qty_returned);
                    }
                } else {
                     item_discount = parseFloat(ds/qty_returned);
                }
											...
											<div class="col-sm-12">
												<div class="col-md-4">
													<div class="form-group">
														...
														<?= form_input('payment_reference_no', ($payment ? $payment->reference_no : $payment_ref), 'class="form-control tip" id="payment_reference_no" required="required"'); ?>
													</div>
												</div>
												<div class="col-sm-4">
													<div class="payment">
												...
												<div class="col-sm-4">
													<div class="form-group">
														...
														<select name="paid_by" id="paid_by_1" class="form-control paid_by">
															<option value="cash" <?= ((isset($payment) && $payment->paid_by == 'cash') ? 'selected' : '') ?> ><?= lang("cash"); ?></option>
															<option value="gift_card" <?= ((isset($payment) && $payment->paid_by == 'gift_card') ? 'selected' : '') ?> ><?= lang("gift_card"); ?></option>
															<option value="Cheque" <?= ((isset($payment) && $payment->paid_by == 'Cheque') ? 'selected' : '') ?> ><?= lang("cheque"); ?></option>
															<option value="other" <?= ((isset($payment) && $payment->paid_by == 'other') ? 'selected' : '') ?> ><?= lang("other"); ?></option>
														</select>
													</div>
												</div>
											</div>
											<div class="col-sm-12">
												<div class="col-sm-4" id="bank_acc">
													<div class="form-group">
														...
														echo form_dropdown('bank_account', $bank, (isset($payment) ? $payment->bank_account : ''), 'id="bank_account_1" class="ba form-control kb-pad bank_account" required="required"');
														?>
													</div>
												</div>
											</div>
