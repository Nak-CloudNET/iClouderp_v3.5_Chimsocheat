	...
	public function edit($id = null)
    {
        ...
        if ($this->form_validation->run() == true && $this->quotes_model->updateQuote($id, $data, $products, $payment)) {
            ...
            foreach ($inv_items as $item) {
                ...
                if ($row->tax_rate) {
                    $tax_rate = $this->site->getTaxRateByID($row->tax_rate);
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'group_prices' => $group_prices, 'all_group_price' => $all_group_prices, 'dropdown_group_prices' => $dropdown_group_prices);
                } else {
                    $pr[$ri] = array('id' => $c, 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => false, 'options' => $options,'group_prices' => $group_prices, 'all_group_price' => $all_group_prices, 'dropdown_group_prices' => $dropdown_group_prices);
                }
                $c++;
            }
    ...
    public function suggestions()
    {
        ...
        if ($rows) {
            foreach ($rows as $row) {
				...
				$row->price_id = $customer->price_group_id;
                ...
                if ($row->tax_rate) {
					...
					$pr[] = array('id' => str_replace(".", "", microtime(true)), 'item_id' => $row->id, 'label' => $row->name . " (" . $row->code . ")", 'row' => $row, 'combo_items' => $combo_items, 'tax_rate' => $tax_rate, 'options' => $options,'warehouse_id'=>$warehouse_id,'customer_id'=>$customer_id,'group_prices'=>$group_prices, 'all_group_price' => $all_group_prices, 'dropdown_group_prices' => $dropdown_group_prices, 'makeup_cost'=>$customer_group->makeup_cost, 'customer_percent' => $customer_group->percent,'makeup_cost_percent'=>(isset($percent->percent)?$percent->percent:0));
