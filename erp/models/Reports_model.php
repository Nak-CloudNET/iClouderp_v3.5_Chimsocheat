...
ORDER BY
    ...
    public function getTotalSales($start, $end)
    {
        ...
        $this->db
             ->select('count(erp_sales.id) as total, sum(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
    ...
    public function getTotalPurchases($start, $end)
    {
        ...
        $this->db
             ->select('count(erp_purchases.id) as total, sum(COALESCE(grand_total, 0)) as total_amount, SUM(COALESCE(paid, 0)) as paid, SUM(COALESCE(total_tax, 0)) as tax', FALSE)
...
*/
    ...
    public function getStaffDailyPurchases($user_id, $year, $month, $warehouse_id = NULL)
    {
        ...
        $myQuery .= " erp_purchases.biller_id = {$user_biller_id} AND created_by = {$user_id} AND DATE_FORMAT( date,  '%Y-%m' ) =  '{$year}-{$month}'
            GROUP BY DATE_FORMAT( date,  '%e' )";
        $q = $this->db->query($myQuery, false);
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as $row) {
    ...
    public function getStaffMonthlyPurchases($user_id, $year, $warehouse_id = NULL)
    {
    	...
    	if ($user_biller_id) {
	        ...
	        $myQuery .= " erp_purchases.biller_id = {$user_biller_id} AND created_by = {$user_id} AND DATE_FORMAT( date,  '%Y' ) =  '{$year}'
	            GROUP BY date_format( date, '%c' ) ORDER BY date_format( date, '%c' ) ASC";
    	} else {
    		$myQuery = "SELECT DATE_FORMAT( date,  '%c' ) AS date, 
			SUM( COALESCE( product_tax, 0 ) ) AS tax1, 
